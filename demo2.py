...
object_config = {
	"model_path": './ObjectDetector/models/yolov8n-coco.onnx',
	...
	try:
		while cap.isOpened():
			...
			if ret:	
				...
				frame_idx += 1
				if frame_idx % 10 == 0:
					LOGGER.info(f"Processing Frame {frame_idx}... (Last Inference: Obj {obect_infer_time}s, Lane {lane_infer_time}s)")
		...
		LOGGER.info("Interrupted by user. Closing simulation...")

	finally:
		vout.release()
		cap.release()
		cv2.destroyAllWindows()
		LOGGER.info("Simulation finished and resources released.")
